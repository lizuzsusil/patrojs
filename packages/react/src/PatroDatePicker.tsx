...
import type { PatroJsDateProps, PatroJsLanguage, PatroJsDay } from '@patrojs/core';
import type { PatroJsFormatter, PatroJsProps } from './types';
import { useStyleResolver } from './useStyleResolver';
import { isDisabledDate, findNext, bsDateEqual } from './utils';
    ...
    return (
      ...
      >
        {weekDays.map((d) => (
          <div
            key={d}
            className={`pjs-weekday ${classNames.dayLabel || ''}`.trim()}
  ...
  const portaledDropdown = open && portal && portalStyle && createPortal(
    <div
      ...
      style={{
        ...
        ...(!minimal ? { '--pjs-primary': s.colors.primary!, '--pjs-primary-fg': s.colors.primaryBg!, '--pjs-today-border': s.colors.todayBorder ?? s.colors.primary! } as CSSProperties : undefined),
        ...styles.dropdown,
      }}
      role="dialog"
      aria-label={isNepali ? 'मिति चयन गर्नुहोस्' : 'Date picker'}
