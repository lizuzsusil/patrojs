...
/**
 * Converts English (Latin) digits to Nepali numerals.
 *
 * @param value - A number or numeric string.
 * @returns The value represented using Nepali digits.
 */
...
/**
 * Converts a string containing Nepali numerals to
 * its numeric (English digit) equivalent.
 *
 * @param value - A string containing Nepali digits.
 ...
 * @param language - Output language (`en` or `ne`).
...
/**
 * Returns the localized short weekday label.
 *
 * Day index follows the JavaScript convention:
 * `0 = Sunday`, `6 = Saturday`.
 *
 * @param dayIndex - Weekday index.
 * @param language - Output language (`en` or `ne`).
 * @returns The localized weekday abbreviation.
 */
...
/**
 * Formats a Bikram Sambat date as a standard
 * `YYYY-MM-DD` string using the selected language.
 *
 * English uses Latin digits, while Nepali uses
 * Nepali numerals.
 *
 * @param year - BS year.
...
/**
 * Formats a Bikram Sambat date using a configurable
 * output pattern.
 *
 * Supports predefined patterns as well as custom
 * format strings containing tokens such as
 * `YYYY`, `MM`, `DD`, `MMMM`, `M`, and `D`.
 *
 * @param date - The BS date to format.
 * @param language - Output language.
 * @param pattern - Desired format pattern.
 * @returns The formatted date string.
 */
...
/**
 * Creates the default formatter used by the date picker.
 *
 * The returned formatter provides helper methods for:
 * - Formatting BS dates
 * - Parsing user input
 * - Validating date strings
 * - Generating localized placeholders
 *
 * @returns A formatter implementation with `format`,
 * `parse`, `isValid`, and `placeholder` methods.
 */
